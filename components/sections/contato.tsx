...
export function Contato() {
  ...
  return (
    <section id="contato" className="scroll-mt-20 bg-creme py-20 sm:py-28">
      <div className="container-x grid gap-12 lg:grid-cols-2 lg:gap-16">
        <Reveal>
          <div>
            ...
            <div className="mt-8 flex items-start gap-3 text-ink/70">
        ...
        <Reveal delay={0.1}>
          <div className="rounded-3xl border border-creme-300 bg-creme-200/60 p-8">
            <h3 className="font-display text-2xl text-ink">Horário de funcionamento</h3>
            <dl className="mt-6 space-y-3">
              {horarioResumo.map((linha) => (
                <div
                  key={linha.dias}
                  className="flex items-baseline justify-between gap-4 border-b border-creme-300 pb-3 last:border-0 last:pb-0"
                >
                  <dt className="text-ink/70">{linha.dias}</dt>
                  <dd className="tabular-nums text-ink">{linha.horas}</dd>
                </div>
              ))}
            </dl>
