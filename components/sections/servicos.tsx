              ...
              <Reveal key={categoria.id} delay={Math.min(i, 3) * 0.05}>
                <div className="grid gap-6 lg:grid-cols-[16rem_1fr] lg:gap-10">
                  <div>
                    ...
                    <h3 className="mt-4 font-display text-3xl text-ink">{categoria.nome}</h3>
                    <p className="mt-2 text-sm text-ink/50">{categoria.resumo}</p>
                  </div>
                  ...
                  <ul className="grid gap-4 sm:grid-cols-2">
                    {lista.map((servico) => (
                      <li
                        key={servico.id}
                        className="flex flex-col rounded-2xl border border-creme-300/70 bg-white/60 p-6 transition duration-200 ease-suave hover:border-terracota/40 hover:shadow-[0_12px_40px_-24px_rgba(23,16,13,0.5)]"
                      >
                        <div className="flex items-start justify-between gap-4">
                          <h4 className="font-medium text-ink">{servico.nome}</h4>
                        ...
                        {(servico.inclui ?? servico.opcoes) && (
                          <ul className="mt-4 flex flex-wrap gap-1.5">
                            {(servico.inclui ?? servico.opcoes)!.map((item) => (
                              <li
                                key={item}
                        ...
                        <div className="mt-6 flex items-center justify-between border-t border-creme-300/70 pt-4">
                          <span className="text-xs tracking-wider text-ink/40">
                            ≈ {duracaoLabel(servico.duracaoMin)}
                          </span>
                          <BotaoAgendar servicoId={servico.id} nomeServico={servico.nome} />
                        </div>
                      </li>
                    ))}
                  </ul>
